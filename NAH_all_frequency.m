%% code for pressure, velocity and intensity calculation for all frequencies
...
reconstruction_radius=150:0.5:150.5;    % radius in mm, change it as per need
...
P(:,:,ii)=abs(Gp).*ftout;
P1(:,:,ii)=(ifft2(P(:,:,ii)));
P2(:,:,ii)=P1(n_zeropad+1:MEFTz-n_zeropad,:,ii);             
...
V(:,:,ii)=abs(Gv).*ftout;
V1(:,:,ii)=(ifft2(V(:,:,ii)));
V2(:,:,ii)=V1(n_zeropad+1:MEFTz-n_zeropad,:,ii);
...
%I1=P1.*V1;
I1(:,:,ii)=abs(P1(:,:,ii)).*abs(V1(:,:,ii));
I2(:,:,ii)=I1(n_zeropad+1:MEFTz-n_zeropad,:,ii);

count
end

% calculating total pressure due to all frequency components
P3=abs(P2);
V3=abs(V2);
I3=abs(I2);
P4=zeros(33,49);
...
for i=1:1:sampling_fourier
    if P3(1,1,i)>0
        p_vector(i)=1;
        P4(:,:)=P4(:,:)+P3(:,:,i).^2;                   % total Prms^2 
        V4(:,:)=V4(:,:)+V3(:,:,i).^2;                   % total Vrms^2
        I4(:,:)=I4(:,:)+I3(:,:,i);                      % total Irms
    else
        p_vector(i)=0;    
    end
end
    
P5(:,:,count)=sqrt(P4);        % Prms
V5(:,:,count)=sqrt(V4);        % Vrms
I5(:,:,count)=I4;

% calclating the pressure in dB due to all the frequencies

PdB(:,:,count)=20*log10(P5(:,:,count)*(10^6)/20);       
clf
figure(2)
axes1 = axes('Parent',figure(2),'YTickLabel',{'0','10','20','30','40'},...
    'YTick',[1 9 17 25 33],...
...
contour(PdB(:,:,count),'LineColor',[0 0 0],'Fill','on','Parent',axes1);
colorbar('peer',axes1);

% calculating the velocity in dB due to all the frequencies

VdB(:,:,count)=20*log10(V5(:,:,count)*(10^8)/5);

% calculating the intensity in dB due to all the frequencies

IdB(:,:,count)=10*log10(I5(:,:,count)*(10^12));
end
...
lemon=PdB(:,plot_theta,:); % (z,theta,r)
lemon1=size(lemon);
